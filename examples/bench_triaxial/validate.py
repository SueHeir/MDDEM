...
if phi_values and all(15 <= p <= 40 for p in phi_values):
    print(f"  [3] Friction angle:      PASS (φ = {phi_mean:.1f}° ± {np.std(phi_values):.1f}°, range 15–40°)")
    passed += 1
else:
    print(f"  [3] Friction angle:      FAIL (φ values: {[f'{p:.1f}' for p in phi_values]}°, expected 15–40°)")
