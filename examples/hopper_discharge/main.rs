//! Hopper discharge with mass flow rate measurement.
//!
//! A classic DEM validation case: particles fill a slot hopper under gravity,
//! then a blocker wall is removed and particles discharge through the orifice.
//! A measurement plane at the outlet tracks the mass flow rate, which can be
//! compared against the Beverloo correlation:
//!
//!   W = C * ρ_bulk * sqrt(g) * (D - k*d)^(5/2)
//!
//! where D = orifice width, d = particle diameter, C ≈ 0.58, k ≈ 1.4.
//!
//! ```bash
//! cargo run --example hopper_discharge --no-default-features -- examples/hopper_discharge/config.toml
//! ```
